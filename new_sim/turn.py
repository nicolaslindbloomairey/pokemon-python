...
Functions defined in this file are either:
    * the do_turn method
...
def do_turn(self):
    ...
    action_queue : List[Tuple[float, Action]] = []
    for player in [self.p1,self.p2]:
        for i in range(len(player.active_pokemon)):
            poke = player.active_pokemon[i]
            choice = player.choice[i]
            move = create_move(poke, choice)
    ...
    #check for a winner
    if not self.p1.pokemon_left():
        self.ended = True
        self.winner = 'p2'
    if not self.p2.pokemon_left():
        self.ended = True
        self.winner = 'p1'
    ...
    if self.pseudo_turn:
        ...
        for i in range(len(self.p1.active_pokemon)):
            if self.p1.active_pokemon[i].fainted:
                self.p1.request[i] = 'switch'
            else:
                self.p1.request[i] = 'pass'

        # check player 2's pokemon
        for i in range(len(self.p2.active_pokemon)):
            if self.p2.active_pokemon[i].fainted:
                self.p2.request[i] = 'switch'
            else:
                self.p2.request[i] = 'pass'
    return

def create_move(p : Pokemon, c : Decision) -> dex.Move:
    '''
    ...
    if not p.can_z(move) or not c.zmove:
        return move
...
def populate_action_queue(q : List, p : Pokemon, c : Decision, m : dex.Move) -> None:
    ...
    # move decision actions
        ...
        a = dex.Action('move', user=p, move=m, target=c.target)
...
def run_action(self, a : Action) -> None:
    ...
    if a.action_type == 'switch':
        if a.user.player_uid == 1:
            p1.switch(a.user, a.pos)
        if a.user.player_uid == 2:
            p2.switch(a.user, a.pos)
        return
    ...
    if not self.doubles and a.target[0:3] == 'foe':
        p = a.user.player_uid
        if p == 1:
            run_move(a.user, a.move, p2.active_pokemon[0])
        if p == 2:
            run_move(a.user, a.move, p1.active_pokemon[0])
        return
